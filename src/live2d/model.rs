...
use anyhow::anyhow;
use cubism::{core::{Drawable, DynamicFlags}, model::UserModel};
use glium::{Display, IndexBuffer, Texture2d, VertexBuffer, glutin::surface::WindowSurface, texture::RawImage2d};

use crate::live2d::{clipping_manager::ClippingManager, vertex::Vertex};
...
impl Model {
  pub fn new(display: &Display<WindowSurface>, model_path: PathBuf) -> anyhow::Result<Self>{
    let model_name = model_path.file_name().ok_or_else(|| anyhow!("Fail to get model name from '{}'", model_path.display()))?;
    ...
    let mut indice_buffers = Vec::new();

    vertex_buffers.reserve(cubism.drawable_count());
    indice_buffers.reserve(cubism.drawable_count());
    ...
    } in cubism.drawables()
    {
      let buffer: Vec<_> = vertex_positions
        .iter()
        .zip(vertex_uvs)
        .map(|(&a_position, &a_texCoord)| Vertex {
          a_position,
          a_texCoord,
        })
        .collect();
    ...
    let clipping_manager = ClippingManager::new(display, &cubism, 4)?;
